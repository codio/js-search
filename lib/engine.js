...
(function (mod) {
    ...
    function getToken(self, term) {
        var regex = /\:(?=([^"\\]*(\\.|"([^"\\]*\\.)*[^"\\]*"))*[^"]*$)/g;
        var tmpTerm = term.replace(regex, '▐');
        var parts = tmpTerm.split('▐');
    ...
    p.isSubsetFilter = function (str1, str2) {
        ...
        // if extend and diff contains quotes - start new search
        var isDifContainsQuotes = false;
        if (isExtend) {
            isDifContainsQuotes = /['"]/.test(str2.replace(str1, ''));
        }

        return !isDifContainsQuotes &&
            isExtend && str1.split(':').length === str2.split(':').length;
    };
