(function (mod) {
    ...
    if (typeof exports === 'object' && typeof module === 'object') { // CommonJS
        module.exports = mod(require('./engine'), require('immutable'));
    }
    else if (typeof define === 'function' && define.amd) { // AMD
        define(['./engine', 'immutable'], mod);
    }
    else { // Plain browser env
        mod();
    }
})(function (engine, Immutable) {
    'use strict';
    ...
    p.strongSearch = function (terms, results) {
        ...
        for (var f = 0; f < this._filters.length; f++) { // loop filters
            ...
            for (var i = output.size - 1; i >= 0; i--) { // loop output
                var item = output.get(i);
    ...
    p.softSearch = function (terms, results) {
        var output = Immutable.List();
        if (terms.length === 0) {
            return results.slice(0);
        }
        for (var f = 0; f < this._filters.length; f++) { // loop filters
            var filter = this._filters[f];
            for (var i = results.size - 1; i >= 0; i--) { // loop results
                var item = results.get(i);
                var match = filter.checkMatchItem(terms, item);
                if (match) {
                    output = output.push(results.get(i));
                    results.splice(i, 1);
                }
                if (results.size === 0) {
                    break;
